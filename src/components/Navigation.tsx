...
export function Navigation({ activeTab, onSelectTab }: NavigationProps) {
  const tabs: { key: TabKey; label: string; icon: (active: boolean) => React.ReactNode }[] = [
    {
      ...
      label: "Home",
      icon: (active) => (
        <svg className={`w-4 h-4 transition-colors ${active ? "text-[#111111]" : "text-[#8E8E93]"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      <div className="flex items-center gap-1 bg-[#141414] px-2 py-1.5 rounded-full dock-elevation border border-white/10 transition-all duration-200">
        {tabs.map((tab) => {
